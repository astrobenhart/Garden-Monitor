...
class Genplots():
	...
	def gen_plots(self):
		...
		self.data.humidity.plot(ax=ax[0][0])
		ax[0][0].set_title('Humidity')
		ax[0][0].set_xlabel('Datetime')
		ax[0][0].set_ylabel('Humidity (%)')

		self.data.air_temp1.plot(ax=ax[0][1], label='AM2320', color='b')
		self.data.air_temp2.plot(ax=ax[0][1], label='BMP180', color='orange')
